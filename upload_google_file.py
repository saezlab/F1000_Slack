...
def validate_arguments(args):
    ...
    if args.service_account_file and not os.path.exists(args.service_account_file):
        raise FileNotFoundError(
            f"Service account file not found: {args.service_account_file}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload (update) a file on Google Drive using a service account."
    )
    parser.add_argument(
        "--file_id", required=True, help="The ID of the Google Drive file to update."
    )
    parser.add_argument(
        "--input_path", required=True, help="The local path of the file to upload."
    )
    parser.add_argument(
